...
- [x] 2.2.5 Ontvlecht `audit/finding_classification.py` en `audit/finding_classification_20260420.py`: documenteer in commit-message dat `_20260420.py` regel 645 importeert uit het oude bestand; consolideer naar `src/iso_audit/classification/findings.py` met behoud van beide functionaliteiten — Geconsolideerd op `refactor/iso-audit-milestone-b`. v2 als basis (caching, Kostenteller, rehash, dry-run-cost, mis-tag-filter, sharpness-prompts) + `review_en_bevestig` uit v1 inline. Legacy `sla_op_in_sheets`-wrapper geschrapt (callers importeren uit `iso_audit.reporting.sheets_gws`). 43 tests; per-file `E501`-ignore voor lange system-prompts; Drive-CLI valt graceful terug op importlib als §2.3.2 nog niet beschikbaar is.
